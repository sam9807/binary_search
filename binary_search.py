    ...
    def greaterthan(left, right):
        ...
        if xs[mid] == x:
            if mid == 0 or xs[mid - 1] > x:
                ...
                return greaterthan(left, mid - 1)
        if left == right:
            return None
        if x < xs[mid]:
            return greaterthan(mid + 1, right)
        if x > xs[mid]:
            return greaterthan(left, mid - 1)
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    we will spend a lot of time talking about different f functions that can be
    minimized and their applications.
